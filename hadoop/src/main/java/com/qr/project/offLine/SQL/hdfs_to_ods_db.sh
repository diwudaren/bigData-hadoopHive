...
load_data(){
    ...
    for tableName in $*;do
        # 判断路径是否存在
        hadoop fs -test -e /origin_data/$APP/db/${tableName:4}/$do_date
        #路径存在方可装载数据
        if [ $? -eq 0 ] ;then
            sql=$sql"load data inpath '/origin_data/$APP/db/${tableName:4}/$do_date' OVERWRITE  into table $APP.$tableName partition(dt='$do_date');"
        fi
    done
    echo $sql
    hive -e "$sql"
